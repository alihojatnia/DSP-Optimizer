...
with tab2:
    ...
    with col1: ph = st.slider("pH", 5.0, 9.0, 7.0)
    with col2: nacl = st.slider("NaCl (mM)", 0, 1000, 250)
    with col3: load = st.slider("Load (mg/ml)", 10, 150, 50)
    
    if st.button(" Generate DoE Plan", type="primary"):
        factors = {'pH': [ph-0.5, ph+0.5], 'NaCl_mM': [nacl-100, nacl+100], 'Load_mg_ml': [load-20, load+20]}
...
st.markdown("---")
st.info("**Fork & Star** | [GitHub](.) | Built for **Novartis Downstream Process Development**")